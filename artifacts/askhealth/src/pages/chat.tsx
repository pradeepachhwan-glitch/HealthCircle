...
import {
  ...
  FileText, Loader2, ShieldCheck, Clock
} from "lucide-react";
...
function VerificationBadge({
  ...
  verifiedByName?: string | null;
  // Emergency replies come from a hard-coded safety template, not from
  // the AI model. We label them differently so we never imply Yukti
  ...
  if (status === "verified") {
    return (
      <div
        className="mt-1.5 inline-flex items-center gap-1.5 rounded-full bg-emerald-50 border border-emerald-200 px-2 py-0.5 text-[11px] font-medium text-emerald-700"
        data-testid="verification-badge-verified"
        title={verifiedByName ? `Reviewed and verified by ${verifiedByName}` : "Reviewed and verified by a medical professional"}
      >
        <ShieldCheck className="w-3 h-3" />
        {verifiedByName ? `Verified by ${verifiedByName}` : "Verified by a doctor"}
      </div>
    );
  }
  if (isEmergencyTemplate) {
    return (
      <div
        className="mt-1.5 inline-flex items-center gap-1.5 rounded-full bg-red-50 border border-red-200 px-2 py-0.5 text-[11px] font-medium text-red-700"
        data-testid="verification-badge-emergency"
        title="This is a hard-coded emergency safety message — not generated by the AI and not yet reviewed by a physician."
      >
  ...
  return (
    <div
      className="mt-1.5 inline-flex items-center gap-1.5 rounded-full bg-amber-50 border border-amber-200 px-2 py-0.5 text-[11px] font-medium text-amber-700"
      data-testid="verification-badge-pending"
      title="Yukti's reply has not been reviewed by a physician."
    >
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      <div className={`max-w-[80%] ${isUser ? "items-end" : "items-start"} flex flex-col`}>
        ...
        {!isUser && (
          // Honest default: any assistant message (including older ones with
          // a null status from before this feature shipped) is treated as
          // "pending" until a doctor explicitly verifies it. A null status
          // is NOT the same as "no claim" — without this fallback, old AI
          // answers would silently appear unmarked, which understates that
          // they were not physician-reviewed. Emergency replies use a
          // template, not the AI model — surfaced via a distinct badge.
          <VerificationBadge
            status={message.verificationStatus === "verified" ? "verified" : "pending"}
            verifiedByName={message.verifiedByName}
            isEmergencyTemplate={message.intent === "emergency"}
          />
        )}

...
export function useTeleconsultCall(
  ...
  // Fetch professional ICE configuration (STUN + TURN)
  useEffect(() => {
    if (enabled && consultationId) {
      fetch("/api/tc/ice-servers", { credentials: "include" })
        .then(r => r.json())
        .then(data => {
          if (data.iceServers) setIceServers(data.iceServers);
        })
        .catch(err => console.warn("[tc-call] failed to fetch ice servers, using fallback", err));
    }

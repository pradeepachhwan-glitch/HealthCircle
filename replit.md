...
- `artifacts/askhealth/` — Main React frontend (previewPath: `/`). Installable PWA — `manifest.json`, service worker (`/sw.js`), icons (`/icon.svg`, `/icon-192.png`, `/icon-512.png`, `/apple-touch-icon.png`), offline fallback (`/offline.html`). Install button in `src/components/PWAInstallButton.tsx` uses `beforeinstallprompt` for Chromium one-tap install and falls back to a state-aware sheet otherwise. SW registers only in `import.meta.env.PROD` to avoid intercepting the workspace dev iframe. **Critical rendering note**: the install instructions modal MUST render via `createPortal(..., document.body)` — `SiteHeader` uses `backdrop-blur-md` (= `backdrop-filter`), which CSS-spec creates a containing block for any fixed-positioned descendants. Without the portal, a `fixed inset-0` overlay rendered inside the header gets trapped inside the 64px-tall sticky header strip and looks like nothing happened when the button is clicked. The sheet uses `max-h-[calc(100dvh-...)] overflow-y-auto` + safe-area padding so it never overflows on short / landscape phones.
  - **Install sheet has three honest states** chosen by `sheetState`: (1) `in-app` for Instagram/Facebook/TikTok/LinkedIn/X/WeChat/Snapchat/Line/KakaoTalk webviews where install is impossible — shows an amber warning + an "Open in browser" CTA + a "Copy link to paste in your browser" fallback (since `window.open` from inside a webview often just opens another in-app tab); (2) `framed` for the Replit preview iframe — same open-in-browser CTA without warning copy; (3) `manual` for normal browsers that haven't yet fired `beforeinstallprompt` — shows visual mockups of where to tap (Safari Share icon for iOS, Chrome ⋮ menu for Android, address-bar install icon for desktop), each highlighted with a pulsing brand-colored ring. The `beforeinstallprompt` one-tap path bypasses the sheet entirely and calls `prompt()` directly; if `prompt()` ever throws, we fall back to the sheet so a tap is never a no-op. Detection helpers: `detectPlatform`, `detectInAppBrowser`, `isStandalone`, `isInIframe`.
...
- **Mobile shell respects iOS notch + home bar.** `index.css` defines real safe-area utilities (`.pb-safe`, `.pt-safe`, `.pb-safe-nav`) plus `.layout-mobile-shell` and `.layout-mobile-header` — the latter two embed a `@media (min-width:768px)` reset to 0 in the same rule, because Tailwind's `md:pt-0` cannot override an inline `style` attribute (specificity loss). `Layout.tsx` uses `.layout-mobile-shell` for body padding (header height + notch on top, BottomNav height + home bar on bottom) and `.layout-mobile-header` for the fixed header's notch-aware min-height. `BottomNav.tsx` was previously referencing a class `safe-area-pb` that didn't exist anywhere in the codebase — replaced with the real `pb-safe` so iPhone X+ home indicators no longer overlap the nav buttons. Bottom nav items now have `min-h-[48px]` and the mobile header hamburger / logo have `min-h-[44px] min-w-[44px]` to clear the 44px Apple HIG / WCAG touch-target floor; hamburger also gained `aria-label="Open menu"`. The mobile sidebar Sheet widened from `w-64` to `w-72` for better thumb reach.
